...
type Server interface {
	...
	QueryIP(ctx context.Context, domain string, clientIP net.IP, option dns.IPOption, disableCache bool, bypassCacheIferr bool) ([]net.IP, error)
}
...
func (c *Client) QueryIP(ctx context.Context, domain string, option dns.IPOption, disableCache bool, bypassCacheIfErr bool) ([]net.IP, error) {
	ctx, cancel := context.WithTimeout(ctx, 4*time.Second)
	ips, err := c.server.QueryIP(ctx, domain, c.clientIP, option, disableCache, bypassCacheIfErr)
	cancel()
